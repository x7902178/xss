...
import torch
from ultralytics.nn.modules.block import Bottleneck,SPPF
from ultralytics.nn.modules.conv import Conv,C2f_CA
...
def prune(m1, m2):
    if isinstance(m1, C2f_CA):  # C2f as a top conv
        m1 = m1.cv2
    ...
    for i, item in enumerate(m2):
        if isinstance(item, C2f_CA) or isinstance(item, SPPF):
            m2[i] = item.cv1
